...
def self_test(expected):
    mutations = [
        ...
        # Resealed JSON type aliases must not pass Python's numeric equality.
        lambda p: p.__setitem__('source_cutoff', float(LIMIT)),
        lambda p: p['raw_tails']['rows'][0].__setitem__('clock', False),
        lambda p: p['clock_change_spikes'][0].__setitem__('j', 2.0),
        lambda p: p['limit_controls']['shift_uniform_defect'].__setitem__(0, True),
    ]
    for change in mutations:
        altered = deepcopy(expected)
        change(altered)
        require(json.dumps(altered, sort_keys=True, allow_nan=False) !=
                json.dumps(expected, sort_keys=True, allow_nan=False), 'no-op mutation')
