...
def log_bounds(x: Fraction) -> tuple[Fraction, Fraction]:
    """Range-reduced strict rational bounds for log(x), x > 0."""
    assert x > 0
    ...
    if exponent >= 0:
        return (
            exponent * LOG2_LOWER + lower_y,
            exponent * LOG2_UPPER + upper_y,
        )
    return (
        exponent * LOG2_UPPER + lower_y,
        exponent * LOG2_LOWER + upper_y,
    )
...
def generate() -> dict[str, object]:
    ...
    return {
        ...
        "height_certificate": {
            ...
            "real_gap_upper_fraction": [
                REAL_GAP_UPPER.numerator,
                REAL_GAP_UPPER.denominator,
            ],
