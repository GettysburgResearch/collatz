...
| L-0020 | frequency-block mean, position-free (FBM) | `mean_{θ∈I}|S_K|/2^K ≤ (2/π + 1/81)^r` for EVERY interval I of 81^r consecutive frequencies (complete-residue-system + L-0011 bijection + L-0012 chain) — supplies PR #16's (FBM) hypothesis | PROPOSED | — | `ADDENDA.md` A3 |
...
| T-0012 | Theorem 12 | Fixed-frequency a.e.-depth decay (as stated in `EQ.md`) | **PARTIAL** — displayed proof gapped for 3\|θ (PR #16 audit, verified: first-factor mean 1.0 at θ = 81 vs charged 0.748; statement unrefuted, full-period means far under repaired bounds); repair = PR16/T-9303, exact loss ⌈v₃(θ)/4⌉ | `eq_theorem12.py`; `ADDENDA.md` A2 | `EQ.md`, `ADDENDA.md` A2 |
...
| T-0028 | product rigidity | For every split K = n+j: `81^j A = 64^j A* + 17c_w + r·64^K` (exact, per element) ⟹ R_K's coarse structure at scale 81^{−j} = the room process, fine structure = translate of R_n; sorted embeddings of R_{n+j} and CRT(R_n × C_j) agree within ~1 joint-slot (measured; D* equal to all digits). EQ's new-content-per-scale isolated to the room walk | PROPOSED (complete: word–class–room lemma proved, x_w = C_j verified j ≤ 4, rooms r = t exhaustively verified at two splits; sharp slot bound (81/64)^j − 1 matches measurement exactly) | `eq_joint_discrepancy.py` | `EQ-INTERCHANGE.md` §§16, 19 |
| T-0029 | capacity achievability | `f_L = |D_L|/N_L ≥ c ≈ 2^{−6}` for L ≥ L₀, unconditionally (the L=6 collision lifts: pairs (14+64m, 15+64m) collide at every depth; supercritical continuations = binomial tail, verified exactly 22/176/3473 at L = 12/16/20). Corollary: **cost(L) = (1−H(log₃2))L + Θ(log L) — the cost-floor theorem is two-sided** | PROPOSED | inline check (report, session 8) | `CAPACITY.md` |
| T-0030 | density-one full weighted EQ (cross-branch assembly) | L-0020 + PR16/{L-9304, T-9303, L-9302, T-9302}: for every α < log₈₁√2, `E_K → 0` along density-one depths (quantitative 81^{−cm} rates) ⟹ **fair-window equidistribution + minimal-survivor law for almost every depth**. All links PROPOSED, none conditional. All-K case = the tower (O-0018) | PROPOSED (assembly; numerics confirm T-9303 with wide margins) | period-mean checks in ADDENDA A2/session-11 report | `ADDENDA.md` A4 |
...
| Q-0004 | capacity achievability | Is the collision fraction bounded below? | SUPERSEDED — resolved affirmatively by T-0029 (`CAPACITY.md`); constant-sharpening remains open (bound 2^{−6} vs measured 0.36–0.66) | `CAPACITY.md`; packet **P3** | — |
