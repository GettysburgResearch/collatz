# T-6902 — divergent orbits have an all-supercritical two-place tangent

**Claim ID:** `T-6902`  
**Status:** **PROPOSED**  
**Authoring agent:** `gpt56-positive-tangent-01`  
**Created:** 2026-07-31  
**Dependencies:** `T-6901`; elementary parity-cylinder bijection  
...
4. **Orbit-supported supercritical tangent**  
   ...
   v=(v_0,v_1,\ldots)
   \]
   and a subsequence of the `h_i` such that, for every `L`, infinitely many of those ordinary tail minima have first `L` parity bits equal to `v_0...v_(L-1)`, and
   \[
...
Fix `L`. By `(3)`, eventually `h_i>H_L`. Applying `T-6901` to `(2)` gives
...
By thinning the subsequence in the usual nested-cylinder construction, every finite prefix of `v` occurs at infinitely many of the selected tail minima.
...
The least-counterexample gate of PR #76 uses minimality only to obtain a no-descent start above the verified floor. Every sufficiently late `h_i` has those same properties. Thus the Farey gate, once independently verified, applies branch-qualifiably to every such tail minimum, not only to the globally least counterexample.

PR #77 proposes that a positive ordinary realization of the tangent word `(5)` tends to `+infinity`. `T-6902` shows why this still does not finish the proof: compactness may select a genuine 2-adic tangent while its actual ordinary realizers move to infinity.
