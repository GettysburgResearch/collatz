...
| [T-6242](T-6242-chi-sigma-floor.md) | PROVED | **Moves the frontier on a known open problem.** No counterexample to `chi = sigma` has `chi(n) <= 200000` — so a scan of `2*10^9` integers certifies the conjecture for every `n` *of any size* with small coefficient stopping time. The obstruction sits exactly at the convergent `125743/79335` of `log2(3)`. **Corrects C-6241's headline.** |
