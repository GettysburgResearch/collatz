...
| [T-6245](T-6245-loop-no-longer-needs-a-constant.md) | PROVED (reduction) | **The no-go was an artefact.** R-6171 bounds `c_j` by "odd steps last"; T-6243(a) proves the above-line condition forbids exactly that. So `chi(m)` is bounded below by a **power** of `m`, not `log_{3/2}(m)`, and the loop needs only `nu_L >> L^mu` — no constant to beat. **Q-6174's target `0.415037` is void**; the `0.050044` available already suffices. Every counterexample's orbit minimum is a `chi = sigma` counterexample. Verified `nu_{j-1} > Bmax(j)` for all `j` in `[66, 376]`. Conjecture NOT proved: needs any lower bound on `nu_L`, i.e. `chi(n) = O(log n)`. |
