...
`PROPOSED` means the complete proof is present but has not yet passed such a review — per the
repository owner, an external cross-model review pass is planned for these. No claim here is
...
|----|------|--------|-------------------|----------|
| L-9901 | [L-9901-map-equivalences.md](L-9901-map-equivalences.md) | PROPOSED (review in progress) | p1 / — | C, T, S orbits are equivalent (reaching 1, boundedness, cycles); every integer T-orbit is trivial-cycle, nontrivial-cycle, or divergent — with the pigeonhole step marked integer-only |
...
| L-9908 | [L-9908-stopping-density.md](L-9908-stopping-density.md) | PROPOSED | p2 / — | Terras density theorem: d({σ(n) > k}) = s_k·2^{−k} ≤ 2^{−k(1−H(γ))}, with float-free certificate 1−H(γ) > 1937/38800 ≈ 0.0499; density-1 of integers dip below any fixed fraction c of their start (bonus, per fixed c); says nothing about individual orbits (boxed) |
...
| L-9910 | [L-9910-cycle-convergents.md](L-9910-cycle-convergents.md) | PROPOSED | p4 / — | Legendre criterion proved self-contained; cycles with x_min ≥ m² must have K/m a continued-fraction convergent of log₂3 (from above); first 15 CF digits [1;1,1,2,2,3,1,5,2,23,2,2,1,1,55] certified by 30 displayed integer power comparisons; K = 2m impossible for nontrivial cycles |
| L-9911 | [L-9911-minimal-counterexample.md](L-9911-minimal-counterexample.md) | PROPOSED (review in progress) | p6 / — | Conditional structure theorem for μ = min counterexample: orbit floor, congruences, record-word disjunction at every j, mode dichotomy (cycle mode: μ ≤ x_min of every nontrivial cycle; divergent mode: liminf density ≥ γ), and the infinite counterexample preimage tree (exactly issue #25's closure hypotheses) |
| L-9912 | [L-9912-cycle-exponent-statistics.md](L-9912-cycle-exponent-statistics.md) | PROPOSED | p5 / — | Constant-exponent cycles are trivial; every nontrivial cycle has some exponent 1 (hence an element ≡ 3 mod 4), indeed > m/3 of them (floor 7); exponent↔residue dictionary mod 2^{t+1}; **bonus: integrality windows eliminate m ∈ {7, 9, 12} and force K to a singleton for m ∈ {5, 8, 10, 11, 13, 14}** |
| L-9914 | [L-9914-backward-tree-counting.md](L-9914-backward-tree-counting.md) | PROPOSED | p7 / — | Explicit census bound: #{odd n ≤ x reaching 1} ≥ (1/5)·x^{3/10} for all x ≥ 1 (and (17/25)x^{3/10} for the full C-census), by a mod-9-controlled preimage tree; root-independent form isolated for issue #25 |
