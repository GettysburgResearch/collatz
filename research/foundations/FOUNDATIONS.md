...
As of 2026-07-25 **every lemma file in this packet has passed such a review** (14/14); an
external cross-model pass remains welcome and would justify `INDEPENDENTLY_VERIFIED`. No claim here is
...
| L-9913 | [L-9913-cycle-length-lower-bound.md](L-9913-cycle-length-lower-bound.md) | PROPOSED (review in progress) | p8 / — | **Every nontrivial Syracuse cycle has m ≥ 2966 odd elements** (K ≥ 4701, C-cycle length ≥ 7667, all elements > 10⁶) — elementary, no Baker-type input, from the verified floor F = 10⁶ via the fractional-part squeeze 1 − {mα} ≤ εm; F enters as a free parameter (sensitivity 10⁶→2966, 10⁷→10946, 10⁸→15601, 10⁹→47468). In-file sweep X-9913 verifies n ≤ 10⁹, giving the labelled corollary **m ≥ 47468, K ≥ 75235, C-length ≥ 122703**. Note: 2966 = 306 + 4·665 is a *semiconvergent* — a convergent-only search would give only q ≥ 1020 |
