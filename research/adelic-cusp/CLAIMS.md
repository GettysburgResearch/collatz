...
| `Q-9302` | Open question | `IDEA` | Two-place proof mechanism after target equivalence | `T-9304`, `L-9306`, `L-9307`, `T-9305` |
...
- `D-9303`, `L-9305`, `T-9304`, `L-9306`, `L-9307`, and `T-9305` are self-contained. The ordered-position consequence of `T-9304` remains explicitly conditional on the branch-qualified issue-#4 room/CRT comparison.
- `T-9305` directly compares the **absolute Fourier criteria** of the CRT product and the actual depth-`K` survivor product. In every range `H=o(64^K)`, no branch-qualified position theorem is needed for that magnitude-level equivalence.
...
- `C-9301`, `Q-9301`, and reframed `Q-9302` are open. `Q-9302` now asks for a genuinely new proof mechanism, not for a weaker decay target.
...
The average-theorem audit verdict is asymmetric:
...
The latest issue-#4 interchange packet defines Cantor classes `C_j`, their transform, a position-rigid CRT comparison with `R_n`, and an inverse-limit room walk. This packet's crosswalk is:
...
- every sub-`64^K` harmonic absolute-Fourier sum for the CRT split is asymptotically equal to the original EQ sum, uniformly in the split = `T-9305`;
- transfer from the CRT set to the **ordered positions** of `R_(n+j)` remains branch-qualified for geometric statements, but it is not needed for `T-9305`'s magnitude comparison;
- `Q-9302` isolates the remaining methodological question: can signed dynamics, a bilateral inverse theorem, or a positive room transfer operator prove the equivalent criterion?