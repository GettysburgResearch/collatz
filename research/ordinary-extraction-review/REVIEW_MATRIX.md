...
**Reviewed source through:** `6131c4768bf52e866829d1ad8ab69a295a90c801`  
...
| `R-7601` strict-causal foundry no-reduction | **PASSED** | Strict causality gives a unique `2`-adic point, but unrestricted causal operators are surjective onto `Z_2`; the tail-property equivalence preserves rather than reduces the ordinary witness problem. |
...
and a stable minimum belongs to every earlier set by nesting. This is the exact
correction to
...
At binary depth `k`, the two lifts of one residue have the same first `k`
shortcut parity bits and opposite bit at time `k`. A strictly causal rule has
already fixed the required bit from the first `k` input digits, so exactly one
lift is selected. This proves unique recursive closure.

Conversely, for any prescribed `alpha in Z_2`, define the causal rule only on
its distinguished digit prefixes to reproduce `par(alpha)` and choose every
off-path value freely. The unique closure point is then exactly `alpha`, proving
surjectivity and continuum multiplicity. If off-path outputs are set to one,
any finite-change-invariant tail property containing `1^infinity` is uniformly
satisfied precisely when the distinguished ordinary witness already has that
property. Thus unrestricted foundry design is an exact parametrization, not a
reduction.
...
6. `R-7601` does not exclude a sharply restricted causal syntax that itself
   proves eventual-zero digits. Such a theorem would be genuine new arithmetic
   content.
...
strictly causal foundries do not reduce the witness problem;
