...
**Claim ID:** `L-7501`  
**Title:** Every periodic shortcut parity block is either an integral positive cycle or has exponentially escaping canonical roots  
...
5. Let \(r_m\in\{0,\ldots,Q^m-1\}\) be the canonical residue realizing the
   first \(m\) copies of \(w\).  If the periodic completion is not a
   nonnegative ordinary integer, then \(r_m\to\infty\) exponentially.
   More precisely:
   ...
   - if \(A<Q\), put \(D=Q-A\).  There is a periodic sequence
     ...
     \(t_m\ge1\) for every \(m\), hence
     \[
     r_m\ge\frac{Q^m+C_w}{D}.
     \tag{6}
     \]
...
boundary: either its least roots stabilize at an exact positive cycle, or they
escape to infinity.  A supercritical periodic block always lies on the negative
boundary face.
...
and 4 now follow immediately from `(3)`.  In the subcritical case, positivity
and ordinary integrality are equivalent to `(4)`.  Because \(x_w\) is the
unique \(2\)-adic realization of \(w^\infty\), an integral value automatically
replays every advertised parity bit and returns after \(L\) steps.

The equality \(A=Q\) cannot occur for \(s,L>0\), because an odd power of three
cannot equal a positive power of two.

## 4. Canonical least-root formulas
...
once \(Q^m>C_w\); the finitely many earlier levels obey the same canonical
formula after direct reduction.  Reducing modulo \(D\) gives the congruence in
`(5)`.  Because \(Q\) is a unit modulo the odd number \(D\), \(Q^{-m}\pmod D\)
is periodic, so \(t_m\) is periodic.  The value \(t_m=0\) is possible exactly
when \(D\mid C_w\).  If that divisibility fails, \(t_m\ge1\), proving `(6)`.
...
- The theorem concerns canonical initial residues, not the magnitude of later
