...
def support_certificate(f:Family,s:int,eta:Interval,rows:list[dict[str,Any]])->dict[str,Any]:
    ...
    for r in range(1,R):
        if product_gate(f,s,r):
            finite.append((r,'nontrivial-product',None))
            continue
        ok,t0=direct_gate(f,s,r,eta)
        if ok:
            finite.append((r,'minimal-positive-total-pulse',t0))
            continue
        raise AssertionError(f'uncovered finite repetition {f.name} s={s} r={r}')
    finite_text=json.dumps(finite,separators=(',',':'))
    finite_summary={
        'count':len(finite),
        'product_count':sum(1 for _,g,_ in finite if g=='nontrivial-product'),
        'direct_count':sum(1 for _,g,_ in finite if g=='minimal-positive-total-pulse'),
        'sha256':hashlib.sha256(finite_text.encode()).hexdigest(),
    }
    ...
    for idx,row in enumerate(rows[:-1]):
        ...
        if margin>0:
            ordinary.append((p,q,qnext))
        else:
            exceptional.append(exceptional_row(f,s,p,q))
    ordinary_text=json.dumps(ordinary,separators=(',',':'))
    ordinary_summary={
        'count':len(ordinary),
        'sha256':hashlib.sha256(ordinary_text.encode()).hexdigest(),
    }
    return {
        'support':s,
        'legendre_start':R,
        ...
        'finite_gate_summary':finite_summary,
        'ordinary_upper_convergent_summary':ordinary_summary,
